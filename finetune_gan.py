...
class GANFineTuner:
    ...
    def generate_sample(self, input_notes, filename):
        ...
        try:
            ...
            if n_vocab == 0:
                print("Warning: No vocabulary found, using default notes")
                pred_notes_mapped = ['C5'] * 100  # Default sequence
            else:
                # Generate sequence
                noise = np.random.normal(0, 1, (1, self.latent_dim))
                predictions = self.generator.predict(noise, verbose=0)
                
                # Convert predictions back to note indices with proper scaling
                # The predictions are in range [-1, 1], we need to map to [0, n_vocab-1]
                pred_notes_mapped = []
                for x in predictions[0]:
                    # Scale from [-1, 1] to [0, n_vocab-1]
            ...
            # Create MIDI file with proper format (list of lists)
            formatted_notes = [[note] for note in pred_notes_mapped]
            create_midi(formatted_notes, filename)
